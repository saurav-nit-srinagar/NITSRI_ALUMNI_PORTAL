      ...
      <div className="flex flex-col lg:gap-1 gap-3 sm:gap-4">
        {
          links.map((link, index) => (
            ...
            >
              {({ isActive }) => (
                <button onClick={() => setShow(false)} className={`text-black w-full flex items-center gap-3 font-medium text-start px-4 py-2 rounded-xl ${isActive ? "bg-[#4B164C] text-white rounded-xl" : "hover:bg-black hover:text-white"}`}>
                  {link.icon} {link.name}
                </button>
              )}
            </NavLink>
