...
const Feedbacks = () => {
    ...
    const { data: bugs, isLoading, isError, refetch, error, } = useQuery({
        queryKey: ["feedback", page, type],
        queryFn: () => getPaginatedBugs("feedback", itemsPerPage, itemsPerPage * (page - 1), type),
    });
    ...
    return (
        <div className="pt-24">
            <Meta name="Feedbacks Report" />
            <Heading heading="Reported Feedbacks"></Heading>
            ...
            <div className="flex flex-col gap-16 pt-10 items-center justify-center">
                {isLoading ? (
                    ...
                    <div className="lg:w-[90%] md:w-[95%] gap-6 w-full m-auto px-5 grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 mb-32 mt-10 place-items-center">
                        {bugs.map((bug, idx) => <FeedbackCard data={bug} key={idx} />)}
                    </div>
                )}
